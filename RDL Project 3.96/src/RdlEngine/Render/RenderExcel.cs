...
namespace fyiReporting.RDL
{
	...
	internal class RenderExcel: IPresent
	{
		...
		{
            _ColSpan = t.ColSpan;      //ÎºÇí¶«

            if (_ColSpan > 1)
            {
                _Excel.AddMergeCell(_ExcelRow, _ExcelCol, _ColSpan);

                Textbox tb = t.ReportItems[0] as Textbox;

                for (int i = 1; i < _ColSpan; i++)
                {
                    if (InTable(tb))
                        _Excel.SetCell(_ExcelRow, _ExcelCol+i, string.Empty, GetStyle(tb,row));
                    else if (InList(tb))
                    {
                        _ExcelCol++;
                        _Excel.SetCell(_ExcelRow, _ExcelCol + i, string.Empty, GetStyle(tb, row));
