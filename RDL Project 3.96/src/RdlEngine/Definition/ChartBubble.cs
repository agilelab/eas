...
namespace fyiReporting.RDL
{
	...
	internal class ChartBubble: ChartBase
	{
		...
		internal ChartBubble(Report r, Row row, Chart c, MatrixCellEntry[,] m) : base(r, row, c, m)
		{
		}
            ...
            for (int iCol = 1; iCol <= SeriesCount; iCol++)
            {
                ...
                for (int iRow = 1; iRow <= CategoryCount; iRow++)
                {
                    ...
                    Point p = new Point(Layout.PlotArea.Left + x, Layout.PlotArea.Top + (maxPointHeight - y));
                        ...
                        if (this.ChartDefn.Subtype == ChartSubTypeEnum.Line || this.ChartDefn.Subtype == ChartSubTypeEnum.SmoothLine || isLine)
                        {
                        ...
                        {  
                    DrawBubble(rpt, g, GetSeriesBrush(rpt, iRow, iCol), p, iRow, iCol, bmin, bmax, bv);
                }
                        lastPoint = p; 
                    }                 
                }
