	...
	internal class ChartLine: ChartColumn
	{
		...
		void DrawPlotAreaLine(Report rpt, Graphics g, double min, double max)
		{
			...
			for (int iCol=1; iCol <= SeriesCount; iCol++)
			{
				for (int iRow=1; iRow <= CategoryCount; iRow++)
				{
					...
					Point p = new Point(x, Layout.PlotArea.Top + (maxPointHeight -  y));
					saveP[iRow-1] = p;
                    DrawLinePoint(rpt, g, GetSeriesBrush(rpt, iRow, iCol), SeriesMarker[iCol - 1], p, iRow, iCol);
				}
                DrawLineBetweenPoints(g, GetSeriesBrush(rpt, 1, iCol), saveP);
			}
			return;
		}
		...
		void DrawLineBetweenPoints(Graphics g, Brush brush, Point[] points)
		{
			...
			try
			{
				...
				p = new Pen(brush, 2);    // todo - use line from style ????
