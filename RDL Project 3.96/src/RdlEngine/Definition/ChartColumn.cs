...
namespace fyiReporting.RDL
{
	...
	internal class ChartColumn: ChartBase
	{
                        ...
                        else //This is a line type plot
                        {
                            if (z == 1)
                            {
                                ...
                                DrawDataPoint(rpt, g, new Point(p.X, p.Y - 14), iRow, iCol); // todo: 14 is arbitrary
                                    DrawLegendLineMarker(g, GetSeriesBrush(rpt, iRow, iCol), new Pen(GetSeriesBrush(rpt, iRow, iCol)), SeriesMarker[iCol - 1], p.X - 5, p.Y - 5, 10);

                                if (LastPoints.ContainsKey(iCol))
                                {
                                    Point[] Points = new Point[2];
                                    Points[0] = p;
                                    Points[1] = (Point)LastPoints[iCol];
                                        // 05052008AJM - Allowing for breaking lines in chart
                                        if (Points[1] != null) {
                                    DrawLineBetweenPoints(g, GetSeriesBrush(rpt, iRow, iCol), Points);
                                }
                                    }
                                LastPoints[iCol] = p;
                            }
        ...
        void DrawLineBetweenPoints(Graphics g, Brush brush, Point[] points)
        {
            ...
            try
            {
                if (brush.GetType() == typeof(System.Drawing.Drawing2D.HatchBrush))
                {
                    ...
                    p = new Pen(new SolidBrush(tmpBrush.ForegroundColor), 2.0F); //1.5F);    // todo - use line from style ????
                }
                else
                {
                    p = new Pen(brush, 2.0F);
                }               
                        ...
                        System.Drawing.Rectangle rect, int plotLeft, int plotRight, out double incr,out int intervalCount)
		{
           ...
           //int intervalCount;
			...
			for (int i=0; i < intervalCount+1; i++)	 
			{
				...
				{
                    Size size = s.MeasureString(rpt, g, v2, TypeCode.Double, null, int.MaxValue);
                    ...
                    s.DrawString(rpt, g, v2, TypeCode.Double, null, vRect);
				}
				else
				{
                    Size size = Style.MeasureStringDefaults(rpt, g, v2, TypeCode.Double, null, int.MaxValue);
					System.Drawing.Rectangle vRect = 
						new System.Drawing.Rectangle(rect.Left+tSize.Width, rect.Top + rect.Height - h - size.Height/2, rect.Width-tSize.Width, size.Height);
                    Style.DrawStringDefaults(g, v2, vRect);
				}
        ...
        {           
            ...
            DrawTitle(rpt, g, a.Title2, new System.Drawing.Rectangle((int)g.VisibleClipBounds.Width - tSize.Width, rect.Top, tSize.Width, rect.Height));
