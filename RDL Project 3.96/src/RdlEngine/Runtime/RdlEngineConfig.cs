...
namespace fyiReporting.RDL
{
	...
	public class RdlEngineConfig
	{
        ...
        static internal IDictionary<string, CustomReportItemEntry> CustomReportItemEntries = null;
  ...
  <CustomReportItems>
    <CustomReportItem>
      ...
      <ClassName>fyiReporting.CRI.BarCode</ClassName>
            ...
            IDictionary<string, CustomReportItemEntry> crieDir =
                new System.Collections.Generic.Dictionary<string, CustomReportItemEntry>(5);	// list of entries
        ...
        static void GetCustomReportItems(IDictionary<string, CustomReportItemEntry> crieDir, XmlNode xNode)
        {
            // loop thru looking to process all the datasource elements
            foreach (XmlNode xNodeLoop in xNode.ChildNodes)
            {
        ...
        static void GetCustomReportItem(IDictionary<string, CustomReportItemEntry> crieDir, XmlNode xNode)
        {
            string type = null;
            string codemodule = null;
            ...
            {   // load the module early; saves problems with concurrency later
                ...
                if (codemodule != null && classname != null)
                {
                    ...
                    foreach (CustomReportItemEntry cri in crieDir.Values)
                    {
                        if (cri.FileName == codemodule && cri.CodeModule != null)
                        {
                            la = cri.CodeModule;
                            break;
                        }
                ...
                crie = new CustomReportItemEntry(type, codemodule, classname, la, msg);
                crieDir.Add(type, crie);
            }
            catch (Exception e)
            {		// keep exception;  if this CustomReportItem is ever used we will see the message
                crie = new CustomReportItemEntry(type, codemodule, classname, null, e.Message);
                crieDir.Add(type, crie);
            }
        }
    ...
    internal class CustomReportItemEntry
    {
        ...
        internal string ErrorMsg;
        internal string FileName;
        internal CustomReportItemEntry(string type, string file, string cname, Assembly codemodule, string msg)
        {
            Type = type;
            CodeModule = codemodule;
            ClassName = cname;
