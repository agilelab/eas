...
namespace fyiReporting.RdlViewer
{
    ...
    public class RdlViewer : System.Windows.Forms.Control
    {
        ...
        private bool _ShowWaitDialog = false;    // show wait dialog when running report
        ...
        public bool ShowWaitDialog
        {
            get
            {
                return _ShowWaitDialog;
            }
            set
            {
                _ShowWaitDialog = value;
            }
        }
        ...
        public void SaveAs(string FileName, string type)
        {
            ...
            if (!(ext == "pdf" || ext == "tif" || ext == "tiff" || ext == "tifbw"))
            {
                ...
                //foreach (DataSource source in this.Report.DataSources)
                //{
                //    must = source.GetConnectionOK();
        ...
        {
            return (int)((double)x * DpiX / POINTSIZEF);
        }

        private int PixelsY(float y)
        {
            return (int)((double)y * DpiY / POINTSIZEF);
        }
	...
	<Body><Height>3 in</Height>
        ...
        private void DrawPanelKeyDown(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyCode == Keys.Down)
            {
                if (!_vScroll.Enabled)
                    return;
                _vScroll.Value = Math.Min(_vScroll.Value + _vScroll.SmallChange,
